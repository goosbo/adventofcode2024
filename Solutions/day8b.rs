...
fn main(){
    ...
    for (_freq, fantennas) in antennas.iter(){
        for i in 0..fantennas.len(){
            antinodes[fantennas[i].0][fantennas[i].1] = '#';
            for j in i+1..fantennas.len(){
                ...
                let mut antinode0 = (coord0.0-delta_i,coord0.1-delta_j);
                while (antinode0.0 >= 0 && antinode0.0 < grid.len() as isize) && (antinode0.1 >= 0 && antinode0.1 < grid[0].len() as isize){
                    antinodes[antinode0.0 as usize][antinode0.1 as usize] = '#';
                    antinode0.0 -= delta_i;
                    antinode0.1 -= delta_j;
                }

                let mut antinode1 = (coord1.0+delta_i,coord1.1+delta_j);
                while (antinode1.0 >= 0 && antinode1.0 < grid.len() as isize) && (antinode1.1 >= 0 && antinode1.1 < grid[0].len() as isize){
